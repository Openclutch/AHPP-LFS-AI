...
namespace InSimDotNet
{
    ...
    public class InSim : IDisposable
    {
        ...
        /// </summary>
        public const int InSimVersion = 9;
        ...
        ///     Set true to release both managed and unmanaged resources; false to release only unmanaged
        ///     resources.
        /// </param>
        ...
        private void PostInitialize()
        {
            ...
            if (Settings.UdpPort > 0) UdpSocket.Bind(Settings.Host, Settings.UdpPort);
        }
        ...
        private static byte[] GetSendBuffer(ISendable[] packets)
        {
            ...
            foreach (var packet in packets) buffer.AddRange(packet.GetBuffer());
        ...
        private ISendable GetMessagePacket(string message, object[] args)
        {
            ...
            var buffer = new byte[MsxLen];
            var length = LfsEncoding.Current.GetBytes(message, buffer, 0, MsxLen);
            if (length < MstLen)
        ...
        public void Bind<TPacket>(PacketHandler<TPacket> callback) where TPacket : IPacket
        {
            ...
            var packetType = PacketFactory.PacketLookup(typeof(TPacket));
        ...
        {
            var e = new PacketEventArgs(packet);

            OnPacketReceived(e);

            if (!e.IsHandled) bindings.ExecuteCallbacks(this, packet);
        }

        private void TcpSocket_PacketDataReceived(object sender, PacketDataEventArgs e)
        {
            var buffer = e.GetBuffer();
            var type = PacketFactory.GetPacketType(buffer);

            if (IsPacketEventNeeded(type))
            {
                var packet = PacketFactory.BuildPacket(buffer);

                if (packet != null) RaisePacketEvent(packet);
            }
        ...
        private void HandleKeepAlive(PacketType type, byte[] buffer)
        {
            ...
            {
                var tiny = new IS_TINY(buffer);
                if (tiny.SubT == TinyType.TINY_NONE) Send(tiny);
            }
        }
        ...
        {
            if (IsConnected) throw new InSimException(StringResources.InSimConnectedMessage);
        }

        [DebuggerStepThrough]
        private void ThrowIfNotConnected()
        {
            if (!IsConnected) throw new InSimException(StringResources.InSimNotConnectedMessage);
        }
