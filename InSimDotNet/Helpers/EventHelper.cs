...
namespace InSimDotNet.Helpers
{
    ...
    internal class EventHelper
    {
        ...
        public event EventHandler<InSimErrorEventArgs> InSimError
        {
            add
            {
                foreach (var conn in connections) conn.InSim.InSimError += value;
            }
        ...
        public event EventHandler<PacketEventArgs> PacketReceived
        {
            add
            {
                foreach (var conn in connections) conn.InSim.PacketReceived += value;
            }
        ...
        public event EventHandler<InitializeEventArgs> Initialized
        {
            add
            {
                foreach (var conn in connections) conn.InSim.Initialized += value;
            }
        ...
        public event EventHandler<DisconnectedEventArgs> Disconnected
        {
            add
            {
                foreach (var conn in connections) conn.InSim.Disconnected += value;
            }
        ...
        {
            foreach (var conn in connections) conn.InSim.Bind(callback);
        }
        ...
        {
            foreach (var conn in connections) conn.InSim.Unbind(callback);
        }
        ...
        public bool IsBoundAll<TPacket>(PacketHandler<TPacket> callback) where TPacket : IPacket
        {
            ...
            if (conn != null) return conn.InSim.IsBound(callback);
            return false;
        }
        ...
        private class Connection
        {
            ...
            public InSim InSim { get; }
            public InSimSettings Settings { get; }
        }
    }
}